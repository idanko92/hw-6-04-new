...
resource "yandex_vpc_security_group" "alpha" {
  name       = "alpha-sg-${var.flow}"
  network_id = yandex_vpc_network.develop.id
  ingress {
...
resource "yandex_lb_target_group" "alpha_tg" {
  name      = "alpha-target-group"
  region_id = "ru-central1"

  dynamic "target" {
    for_each = yandex_compute_instance.alpha
    content {
...
resource "yandex_lb_network_load_balancer" "alpha_nlb" {
  name = "alpha-network-load-balancer"
  ...
  attached_target_group {
    target_group_id = yandex_lb_target_group.alpha_tg.id
